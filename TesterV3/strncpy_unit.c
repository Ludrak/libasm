...
/*   Updated: 2020/11/27 16:45:54 by lrobino          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	test_strncpy_unit(char *dst, char *src, size_t n)
{
	...
	if (n > 0)
	{
		*shared_result[0] = mmap(NULL, sizeof(char) * n, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
		*shared_result[1] = mmap(NULL, sizeof(char) * n, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
	}
	else
	{
		*shared_result[0] = 0;
	...
	write (1, "| ", 2);
	TRY
		*shared_result[0] = strncpy(allocated_args[0], allocated_args[1], n);
	...
	TRY
		*shared_result[1] = ft_strncpy(allocated_args[0], allocated_args[1], n);
		free(allocated_args[0]);
		free(allocated_args[1]);
	CATCH (&sig[1], &error[1]);
	fflush(stdout);
